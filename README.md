...
Step types: `Welcome`, `License`, `Folder`, `Install`, `Finish`

The optional `contentFile` field loads a text file (e.g., `LICENSE.txt`) and displays it in a scrollable box on that step.
...
| `CopyFiles("source", "dest")` | Copy/extract a file from the payload to disk |
